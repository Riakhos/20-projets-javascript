...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof window.API_CONFIG === 'undefined') {
        ...
        console.log('3. Créez un compte gratuit sur https://unsplash.com/developers');
        return;
    }
    
    // Configuration API depuis le fichier externe
    const { API_KEY, API_URL, PER_PAGE = 30, TIMEOUT = 10000 } = window.API_CONFIG;
    
    // Vérification que la clé API est configurée
    if (API_KEY === 'YOUR_API_KEY_HERE' || !API_KEY) {
        console.error('🔑 Clé API non configurée !');
        console.log('📋 Veuillez remplacer YOUR_API_KEY_HERE par votre vraie clé API dans config.js');
        return;
    }
    ...
    async function fetchImages(query = 'random', page = 1) {
        ...
        console.log('🔍 Requête API:', { query, page, url: url.replace(API_KEY, 'HIDDEN_KEY') });
    ...
    function createImageElement(imageData) {
        ...
        img.onerror = () => {
            console.warn('⚠️ Erreur de chargement pour l\'image:', imageData.urls.small);
            img.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="300" height="200"><rect width="300" height="200" fill="%23f0f0f0"/><text x="150" y="100" text-anchor="middle" fill="%23666">Image non disponible</text></svg>';
        };
    ...
    function setupInfiniteScroll() {
        ...
        const observer = new IntersectionObserver((entries) => {
            ...
            if (entries[0].isIntersecting && !isLoading && hasMoreImages) {
                console.log('🔄 Scroll infini déclenché - Chargement de plus d\'images...');
                loadMoreImages();
            }
        }, {
    ...
    async function loadMoreImages() {
        ...
        console.log(`📄 Chargement page ${currentPage} pour "${currentQuery}"`);
    ...
    async function initApp() {
        ...
        try {
            ...
            showError('Erreur lors du chargement de l\'application. Vérifiez votre clé API.');
        } finally {
            toggleLoader(false);
        }
    }
    ...
    async function handleApiError(response) {
        ...
        switch (response.status) {
            case 401:
                errorMessage = '🔑 Clé API invalide ou manquante. Vérifiez votre configuration dans config.js';
                console.error('💡 Solutions possibles:');
                console.error('1. Vérifiez que votre clé API est correcte');
                console.error('2. Créez un nouveau token sur https://unsplash.com/developers');
                console.error('3. Vérifiez que config.js est bien chargé');
                break;
            case 403:
                errorMessage = '⏰ Limite de requêtes atteinte (50/heure en mode démo). Réessayez plus tard.';
                hasMoreImages = false;
                break;
            case 404:
